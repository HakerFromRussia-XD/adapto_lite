...
typedef struct dhcps_msg {
        uint8_t op, htype, hlen, hops;// operation code, hw type, hw addr len, hops
        uint8_t xid[4];// transaction id
        uint16_t secs, flags;           // seconds, flags
        uint8_t ciaddr[4];// client ip addr
        uint8_t yiaddr[4];// your ip addr
        uint8_t siaddr[4];// server ip addr
        uint8_t giaddr[4];// GW ip
